<div>
	...
	<div class="content d-flex flex-column-fluid">
        <div class="container">
			...
			<x-admin.forms.form title="سفارش" :mode="$mode">
				@if ($details->count() > 0)
					<x-admin.forms.input  type="text"  required="true" id="user_number" disabled label="شماره کاربر " wire:model="user_number" />
				@else    
					<x-admin.forms.input  type="text"  required="true" id="user_number" label="شماره کاربر " wire:model="user_number" />
				@endif
				<x-admin.forms.header title="سبد خرید"/>
				<x-admin.button class="btn btn-light-primary font-weight-bolder btn-sm"
                content=" افزودن محصول  " wire:click="addDetails()" />
	...
	<x-admin.modal id="details" size="modal-xl" title="محصول برای سبد خرید">
                ...
                <x-admin.forms.select2 id="product_id" required="true" :data="$data['product']" label="محصول " wire:model="product_id"/>
