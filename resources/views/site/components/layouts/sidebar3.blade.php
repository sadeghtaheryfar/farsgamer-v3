											...
											@foreach($categories as $item)
												<div class="category">
													<div class="d-flex align-items-center justify-content-start" data-id="{{ $item['id'] }}">
														...
														<a href="" class="d-none d-lg-block d-xl-block">{{ $item['title'] }}</a>
														<a class="d-block d-lg-none d-xl-none position-relative" style="z-index: -1;">{{ $item['title'] }}</a>
													</div>
														...
														<div class="sub-categories">
															<div class="sub_category" data-response-id="{{ $item['id'] }}">
																<ul>
																	...
																	<li>
																		<div class="px-2">
																			<a href="{{ route('index.categories',$item2['slug']) }}">{{ $item2['title'] }}</a>
																			<i class="fas fa-chevron-left text-secondary"></i>
																		</div>
																	</li>
																	@endforeach
												...
												<div class="sub_category" id="{{$item[0]['parent_id']}}">
													<ul>
														@foreach($item as $value)
															<li>
																<div class="px-2">
																	<a href="{{ route('index.categories',$value['slug']) }}">{{ $value['title'] }}</a>
																</div>
															</li>
														@endforeach
													</ul>
										...
										<div class="base-category col-12 py-2">
											...
											<div class="category">
												<div class="d-flex align-items-center justify-content-start" data-id="{{ $item['id'] }}">
													...
													<a href="{{ route('index.categories',$item['slug']) }}" class="d-none d-lg-block d-xl-block">{{ $item['title'] }}</a>
													<a href="{{ route('index.categories',$item['slug']) }}" class="d-block d-lg-none d-xl-none position-relative" style="z-index: -1;">{{ $item['title'] }}</a>
												</div>
											</div>
											@endforeach
										</div>
