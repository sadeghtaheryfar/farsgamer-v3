@foreach($form as $key => $item)
	...
	@if($item['type'] == 'range')
        @if(FormBuilder::isVisible($form, $item))
            <div class="col-span-2 lg:col-span-{{$item['width']}}">
                <label for="{{$key}}" class="account-email">{!! $item['label'] !!}</label>
                <input id="{{$key}}" type="number" min="{{ $item['options'][0]['value'] ?? 0 }}" max="{{ end($item['options'])['value'] ?? 1 }}"  name="{{$item['name']}}" class="text-field" placeholder="{{$item['placeholder']}}"
                       wire:model="form.{{$key}}.value" list="list{{$key}}">
                @error('form.'.$key.'.error')
                	<small class="text-red">{{$message}}</small>
                @enderror
            </div>
